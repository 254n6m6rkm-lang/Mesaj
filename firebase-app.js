...
let appData = deepClone(defaultData);
...
function purgeOldLogs() {
  try {
    ...
    appData.logs = appData.logs.filter(log => {
      if (!log || !log.date) return false;
      return String(log.date) >= thresholdStr;
    });
  } catch (e) {
    console.warn('Eski kayıtlar temizlenemedi:', e);
  }
}
...
function setupAuth() {
  ...
  const badgeHome = document.getElementById('btn-home-badge');
  if (badgeHome) {
